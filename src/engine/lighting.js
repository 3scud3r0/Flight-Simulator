...
export function rayleighScattering(wavelengthNm,density=1,pathLength=1){
 const w=wavelengthNm*1e-9,n=1.0003,N=2.547e25;
 const beta=(8*Math.PI**3*(n*n-1)**2)/(3*N*w**4);
 return 1-Math.exp(-beta*density*pathLength);
}
...
export function multipleScattering(singleScatter,albedo=.9,orders=4){
 ...
 for(let i=1;i<orders;i++){term*=clamp(albedo)*.23;radiance+=term}
 return radiance;
}
...
export function volumetricFogRay(ray,steps,sample,{length=1000}={}){
 let T=1,L=[0,0,0];const ds=length/Math.max(1,steps);
...
export function cityNightLights(x,z,floors,time,seed=1){
 ...
 for(let floor=0;floor<floors;floor++)lights.push(hash(x|0,z|0,floor,seed)<night*.68);
 return lights;
}
